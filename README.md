...
## v1.7.3 — Robotics Live ephemeral-token authentication
- Uses the saved Gemini key to provision a short-lived Live API token.
- Connects the Robotics WebSocket using `access_token`.
- Adds clearer token provisioning errors before the socket is opened.


## v1.8.0 — Gemini 3.1 Flash Live + natural interruption
- Default model: `gemini-3.1-flash-live-preview`.
- User speech now interrupts assistant audio (barge-in) instead of disconnecting AI.
